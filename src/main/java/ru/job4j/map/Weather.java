...
public class Weather {
    public static List<Info> editData(List<Info> list) {
        ...
        for (Info elem : list) {
            ...
            int rainfall = elem.getRainfall();
            if (!map.containsKey(elem.getCity())) {
                map.put(city, rainfall);
            } else {
                int tmp = map.get(city);
                map.replace(city, tmp + rainfall);
            }
        }
        for (Map.Entry entry: map.entrySet()) {
        rsl.add(new Info((String) entry.getKey(), (int) entry.getValue()));
        }
        return rsl;
    }
