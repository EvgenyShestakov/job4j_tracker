...
public class SummingMethod {
    ...
    public static Map<String, Integer> summing(List<User> users) {
       return users.stream().flatMap(user -> user.getBills().stream().
                map(bill -> new Pair(user, bill))).
                collect(Collectors.groupingBy(pair -> pair.getUser().
                        getName(), Collectors.summingInt(pair -> pair.getBill().getBalance())));
    }
}
